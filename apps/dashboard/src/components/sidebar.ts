...
export function renderDashboardSidebar(activeRoute: DashboardRouteKey): string {
  const navItems = [
    {
      key: 'overview',
      label: 'Dashboard',
    ...
    },
    {
      key: 'documents',
      label: 'Documents',
      icon: `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z"></path></svg>`,
    },
    {
      key: 'communications',
  ...
  return `
    <aside class="dashboard-sidebar select-none h-full flex flex-col justify-between">
      <div>
        <!-- Royal Square Brand Emblem Header -->
        <div class="px-5 py-6 flex flex-col items-center justify-center border-b border-white/10 text-center">
          ...
          <h1 class="font-serif-royal font-bold text-sm text-white tracking-[0.18em] uppercase leading-tight">
            Royal Square
          </h1>
          <p class="text-[10px] text-slate-400 tracking-[0.25em] uppercase font-medium mt-0.5">
            Financial
          </p>
        </div>
        ...
        <nav class="px-3 py-4 space-y-1">
          ${navItems
            .map((item) => {
              const isActive = activeRoute === item.key || (activeRoute === 'overview' && item.key === 'overview');
              return `
              <button
                data-dash-nav="${item.key}"
                class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs transition-all text-left group ${
                  isActive
          ...
          <div class="pt-2 border-t border-white/10 mt-2">
            ...
            >
              ...
              <span class="text-[10px] text-slate-400 group-hover:text-rose-200 font-mono">Logout</span>
            </button>
          </div>
        </nav>
      </div>

      <!-- Bottom Brand Slogan & Mountain Silhouette -->
